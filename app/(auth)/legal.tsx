...
import { apiPatch, getDriverSessionId, getDriverProfile } from '@/lib/api';
  ...
  const handleAcceptCGU = async () => {
    ...
    try {
      ...
      try {
        ...
        console.log('[LEGAL] Local driver data updated');
      } catch (apiError) {
        ...
        setDriver(updatedDriver);
        console.log('[LEGAL] Local driver data updated despite API error');
      }

      // Rediriger vers l'accueil de l'app chauffeur
      router.replace('/(chauffeur)');
    } catch (error) {
