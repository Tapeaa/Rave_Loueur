...
import { apiFetch, apiPost, getDriverSessionId, setSupportLastSeenId } from '@/lib/api';
...
export default function ChauffeurSupportChatScreen() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await apiPost<{ message: SupportMessage }>(
        '/api/messages/direct/driver/send',
        { content: trimmed },
        {
          headers: {
            'X-Driver-Session': sessionId,
              ...
              >
                <View
                  style={[
                    ...
                    item.senderType === 'admin' ? styles.bubbleAdmin : styles.bubbleDriver,
                  ]}
                >
                  <Text
                    style={
