...
const ONESIGNAL_APP_ID = Constants.expoConfig?.extra?.oneSignalAppId || '62d9a9ec-c62b-4aae-9cb3-e0d0c46ccfe8';
...
/**
 * Définit l'ID externe de l'utilisateur (pour cibler un chauffeur spécifique)
 */
export function setDriverExternalId(driverId: string) {
  const OneSignal = getOneSignal();
  if (!OneSignal || !driverId) return;
  
  try {
    OneSignal.login(driverId);
    console.log('[OneSignal] Set external user ID:', driverId);
  } catch (error) {
...
/**
 * Ajoute un tag au chauffeur
 */
export function addDriverTag(key: string, value: string) {
  const OneSignal = getOneSignal();
  if (!OneSignal) return;
 ...
 */
export function addDriverTags(tags: Record<string, string>) {
  const OneSignal = getOneSignal();
  if (!OneSignal) return;
